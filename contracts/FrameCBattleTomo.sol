...
contract TRC21 is ITRC21 {
    ...
    // Token name
    string public _name = 'Frame C batle';

    // Token symbol
    string public _symbol = 'FCB';
    using SafeMath for uint256;
...
contract FrameCBattle is TRC21, ERC721Full("Frame C batle", "FCB"), Ownable {
    using strings for *;
    ...
    function register(address user, uint _numItems) onlyManager public {
        for (uint j = 0; j < _numItems ; j++) {
            create(user);
        }
    }
    function create(address user) onlyManager public returns(uint){
        _mint(user, ++tokenIdd);
   ...
   function tokenURI(uint256 _tokenId) external view returns (string memory) {
        return tokenMetadataBaseURI.toSlice().concat(uint2str(_tokenId).toSlice());
   }
